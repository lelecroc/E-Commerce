...
<template>
    <div v-for="card in cards">
        <div class="flex" v-show="this.$route.params.name == card.name">
            <div class="w-[50%] flex justify-center">
                <img :src="'.' + card.img" :alt="card.title" class="w-[50%] h-[90%]">
            </div>

            <div class="w-[50%] flex flex-col justify-around">
                <div>
                    <h2 class="text-desktop-2xl font-bold"> {{ card.title }} </h2>
                    <p class="text-desktop-lg"> {{ card.description }} </p>
                </div>
                <!-- BOTTONE -->
